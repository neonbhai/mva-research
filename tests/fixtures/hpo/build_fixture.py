...
Run it with::
    ...
    uv run python tests/fixtures/hpo/build_fixture.py \
        --resources /path/to/hpo-release --out tests/fixtures/hpo

where ``--resources`` holds ``hp.obo``, ``phenotype.hpoa`` and
``genes_to_phenotype.txt`` from https://hpo.jax.org/data/annotations. The release
is 67 MB and is **not** committed; this script cuts a ~600-term slice of the real
graph, with real annotation rows, that is small enough to live in the repository
and still exercises everything the scorer depends on.
...
_REGENERATION_NOTE = (
    ...
    "Regenerate with: uv run python tests/fixtures/hpo/build_fixture.py "
    "--resources <hpo-release-dir> --out tests/fixtures/hpo"
)
...
def build(resources: Path, out: Path) -> None:
    """Write all three fixture files."""
    ontology = HpoOntology.from_obo(resources / "hp.obo")
    ...
    comments, header, rows = _read_rows(resources / "phenotype.hpoa")
    ...
    comments, header, rows = _read_rows(resources / "genes_to_phenotype.txt")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Directory holding hp.obo, phenotype.hpoa and genes_to_phenotype.txt.",
    )
    parser.add_argument(
        "--out",
        type=Path,
